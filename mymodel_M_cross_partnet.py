...
class EachFeatureTrans(nn.Module):
    def __init__(self, dim_head, dropout):
        ...
        self.fc1 = nn.Sequential(nn.Linear(88, 512), nn.Dropout(0.1), nn.GELU())
        self.fc2 = nn.Sequential(nn.Linear(1024, 512), nn.Dropout(0.1), nn.GELU())
        self.fc3 = nn.Sequential(nn.Linear(512, 512), nn.Dropout(0.1), nn.GELU())
        self.fc4 = nn.Sequential(nn.Linear(714, 512), nn.Dropout(0.1), nn.GELU())
        self.fc5 = nn.Sequential(nn.Linear(139, 512), nn.Dropout(0.1), nn.GELU())

        self.encode_au1 = TransformerEncoder(num_patches=96, input_dim=512, depth=1, heads=8, mlp_dim=512, dim_head=dim_head, dropout = dropout)
        self.encode_au2 = TransformerEncoder(num_patches=96, input_dim=512, depth=1, heads=8, mlp_dim=512, dim_head=dim_head, dropout = dropout)
        self.encode_vi1 = TransformerEncoder(num_patches=96, input_dim=512, depth=1, heads=8, mlp_dim=512, dim_head=dim_head, dropout = dropout)
        self.encode_vi2 = TransformerEncoder(num_patches=96, input_dim=512, depth=1, heads=8, mlp_dim=512, dim_head=dim_head, dropout = dropout)
        self.encode_vi3 = TransformerEncoder(num_patches=96, input_dim=512, depth=1, heads=8, mlp_dim=512, dim_head=dim_head, dropout = dropout)
...
class mymodel(nn.Module):
    def __init__(self):
        ...
        self.input_trans = EachFeatureTrans(dim_head=64, dropout=0.1)
        self.partern_trans = EachFeatureTrans(dim_head=64, dropout=0.1)
        self.input_modality_trans = ModalityTrans(dim_head=128, dropout=0.1)
        self.partern_modality_trans = ModalityTrans(dim_head=128, dropout=0.1)
        self.cross_block = Cross_Block(dim_head=128, dropout=0.1)
